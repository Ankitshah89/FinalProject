...
export class ReviewService {
  ...
  public indexReview(){
    return this.http.get<Review[]>(this.url + 'users/reviews')
    .pipe(
      catchError((err: any) =>{
        console.log(err);
  ...
  public businessReviews(bid: number){
    return this.http.get<Review[]>(this.url + 'businesses/' + bid)
    .pipe(
      catchError((err: any) => {
        console.log(err);
        return throwError('Could not retrieve a list of reviews of business by id: ' + bid);
      })
    );
  }
  ...
  public postBusReview(review: Review, bid: number){
    ...
    if(this.authService.checkLogin()){
      return this.http.post<Review>(this.url + 'businesses/' + bid, review, httpOptions)
      .pipe(
        catchError((err: any) => {
          console.log(err);
          return throwError('Could not post New Review to business id: ' + bid);
        })
      )
    } else{
  ...
  public updateBusReview(review: Review, bid: number, rid: number){
    ...
    if(this.authService.checkLogin()){
      return this.http.put<Review>(this.url + 'businesses/' + bid + '/reviews/' + rid, review, httpOptions)
      .pipe(
        catchError((err: any) => {
          console.log(err);
          return throwError('Could not update Review for Business: ' + bid + ' Review: ' + rid);
        })
      )
    };
  }
  ...
  public deleteReview(rid: number){
    ...
    if(this.authService.checkLogin()){
      return this.http.delete<any>(this.url + 'reviews/' + rid, httpOptions)
      .pipe(
        catchError((err: any) => {
          console.log(err);
