...
export class ArticleCommentService {
  ...
  public index() {
    return this.http.get<ArticleComment[]>(this.url + 'comments').pipe(
      catchError((err: any) => {
        console.log(err);
  ...
  public show(aid: number) {
    return this.http.get<ArticleComment[]>(this.url + aid + '/comments').pipe(
      catchError((err: any) => {
        console.log(err);
        return throwError(
          'Could not retrive list of all article comments by article'
        );
      })
    );
  }
  ...
  public replyToCommentOnArticle(
      ...
      return this.http
        ...
        .pipe(
          catchError((err: any) => {
            console.log(err);
            return throwError(
              'Could not post reponse on article ' + aid + 'comment: ' + cid
            );
          })
        );
    } else {
  ...
  public updateCommentById(update: ArticleComment, cid: number) {
    ...
    if (this.authService.checkLogin()) {
      return this.http
        .put<ArticleComment>(this.url + 'comments/' + cid, update, httpOptions)
        .pipe(
          catchError((err: any) => {
            console.log(err);
  ...
  public deleteArticleById(cid: number) {
    ...
    if (this.authService.checkLogin()) {
      return this.http
        .delete<ArticleComment>(this.url + 'comments/' + cid, httpOptions)
        .pipe(
          catchError((err: any) => {
            console.log(err);
